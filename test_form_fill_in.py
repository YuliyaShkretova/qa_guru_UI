...
def test_form_fill_in(browser_setup):
    ...
    #   browser.element('[id="dateOfBirthInput"]').clear().type('13 Nov 1990').press_enter()
    ...
    # browser.element('[id="uploadPicture"]').send_keys(r'C:\..\..\..\sticker.webp')
    browser.element('[id="uploadPicture"]').send_keys(os.getcwd() + '/sticker.webp')
    ...
    browser.all('.table td:nth-child(2)').should(
        have.exact_texts('Юлия Шкретова', 'y.shk@mail.ru', 'Female', '1234567890', '13 November,1990', 'Hindi', 'Music',
                         'sticker.webp', 'Мурталь, Австрия', 'Uttar Pradesh Agra'))
